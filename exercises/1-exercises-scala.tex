...
\setHeader{1. Exercises: Introduction to Scala (WiP)}%
...
\subex{Create a source code directory for our examples:
...
\end{lstlisting}
The resulting \code{Option} object should contain a tuple of values from the Option objects \code{a} and \code{b}, given that both \code{a} and \code{b} are non-empty. Use for-comprehensions
\end{myExercise}

\begin{myExercise}
Implement a check method, which takes a set of values of type \code{T} and a function of type \code{T => Boolean}:
\begin{lstlisting}
def check[T](xs: Seq[T])(pred: T => Boolean): Boolean = ???
\end{lstlisting}
The method must return true if and only if the \code{pred} function returns true for all the values in \code{xs} without throwing an exception. Use the check method as follows:
\begin{lstlisting}
check(0 until 10)(40 / _ > 0)
\end{lstlisting}
\note{The check method has a curried definition: instead of just one parameter list, it has two of them. Curried definitions allow a nicer syntax when calling the function, but are otherwise semantically equivalent to single-parameter list definitions.}
\end{myExercise}
