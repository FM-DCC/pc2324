...
\\\bash{sbt> run}
\\[2mm](You can also \bash{compile}, \bash{~compile}, \bash{console}, and many other.)}
...
\begin{myExercise}
Implement a check method, which takes a set of values of type T and a function of type \code{T => Boolean}:
\begin{lstlisting}
...
\ex{Modify the Pair class from this chapter so that it can be used in a pattern match.
