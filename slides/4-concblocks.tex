...
\code{logMessage}: presented later -- similar to \code{log}, but stores the log message
\end{frame}
...
Illustrated by a bad acquire-release for \code{Copying}, using a mutable \code{n} in:
...
\end{lstlisting}
Optimization: reusing previous \code{Copying} if possible
\end{frame}
...
\begin{frame}\frametitle{Some guidelines to avoid the ABA problem}
  \begin{itemize}
    \item use fresh objects in \code{AtomicReference}
    \item use immutable objects in \code{AtomicReference}
...
\begin{lstlisting}[emph={execute,sleep,log,ConcurrentHashMap,files,Entry}]
...
\begin{lstlisting}[emph={execute,sleep,log,deleteFile,files,prepareForDelete}]
...
...
\begin{frame}\frametitle{Some atomic methods of concurrent Map}
    \centering
