...
\begin{itemize}
  ...
  \item ... if it receives its \code{hello}, it logs and \structure{continues}
  \item ... if it receives something else, it \structure{stops}
  \item \code{context} -- provides core functions, such as \code{stop}
  \item \code{self} -- is the instance's actor reference
\end{itemize}
\end{column}
\end{columns}
\end{frame}
